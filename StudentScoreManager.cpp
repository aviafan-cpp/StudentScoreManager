...
enum pages {choose_res, about, class_information, choose_event_table, edit_event_table, edit_one_student_event_table, score_overview};
pages g_pages;
raylib::Font font;
bool draw_sidebar;
        ...
        window.BeginDrawing();
            ...
            if(draw_sidebar)
            {
                ...
                if(under_result >= 0 || under_result < under_content.size())
                {
                    ...
                    if(under_result == 1) g_pages = about;
                    if(under_result == 2) g_sclass.load_dir(data_dir);
                    if(under_result == 3){
                        g_sclass.save_dir(data_dir);
                        settings.SaveSettingToFile(current_dir + "Data/Settings");
                    }
                    if(under_result == 4) return 0;
                    if(under_result == 5) break;
                }
            }
            ...
            case class_information:{
                ...
                if(button(button_ci).position({g_menu.x.calc_pct(40), g_menu.y.calc_pct(5)}).text("新建").draw().is_clicked())
                {
                    g_sclass.save_dir(data_dir);
                    size_t i = 0;
                ...
                if(result >= 0 && result < names.size())
                {
                    g_sclass.save_dir(data_dir);
                    #if defined(_WIN32)
                        sys_api::open_file("notepad.exe", data_dir + std::to_string(result) + ".ssdata_names");
                    #endif
                    sys_api::message_box("学生分数管理器", "提示：编辑后请保存！\n保存后按“确定”继续。");
                    g_sclass.load_dir(data_dir);
                }
            ...
            case choose_event_table:{
                ...
                if(result >= 0 && result < names.size())
                {
                    g_sclass.save_dir(data_dir);
                    #if defined(_WIN32)
                        sys_api::open_file("notepad.exe", data_dir + std::to_string(result) + ".ssdata_event_table");
                    #endif
                    sys_api::message_box("学生分数管理器", "提示：编辑后请保存！\n保存后按“确定”继续。");
                    g_sclass.load_dir(data_dir);
                }

                if(!settings.IsSettingExist("event_table_num")) settings.AddSetting("event_table_num", 0, sets::Temp);
                    ...
                    for(size_t chlomn = 0; chlomn < 2; ++chlomn)
                    {
                        ...
                        for(size_t i = 0; i < ROW_Y; ++i) // y轴
                        {
                            ...
                            for(size_t j = 0; j < ROW_X; ++j) // x轴
                            {
                                ...
                                if(bt_eet.text(table.at(i + chlomn * ROW_Y).at(j)).position({x, y}).text_size(g_menu.calc_min(35)).draw().is_clicked() && j != 0)
                                {

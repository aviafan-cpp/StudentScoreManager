...
enum pages {choose_res, about, class_information, group_infomation, choose_event_table, edit_event_table, edit_one_student_event_table, score_overview};
pages g_pages;
raylib::Font font;
bool draw_sidebar;
    ...
    while (!window.ShouldClose()) {
        ...
        window.BeginDrawing();
            ...
            if(draw_sidebar)
            {
                ...
                {
                    if(under_result == 0) sys_api::open_program("osk.exe");
                    if(under_result == 1){
                        g_pages = choose_res;
                        draw_sidebar = false;
                    }
            ...
            case group_infomation:{
                ...
                if(settings.GetSettingI("set_student_name") != -1)
                {
                    ...
                    if(clear.is_clicked())
                        input_f.content.codepoints.clear();
                    if(input_f.draw_ok.is_clicked() || IsKeyPressed(KEY_ENTER))
                    {

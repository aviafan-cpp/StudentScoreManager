...
namespace raylib_obj_ex
{
    namespace str_ex
    {
        struct ustring
        {
            ...
            ustring(std::vector<int> other_code_point);
    ...
    namespace input_ns
    {
        struct input
        {
            ...
            size_t index; // 光标在字符的左边，但可以在最后一个字符的右边

            input() : index(0) {}
            input(const input& other) = default;
            input(str_ex::ustring content, size_t index);
            input(std::string content, size_t index);

            bool is_index_correct() const;
            bool is_index_correct(size_t index) const;
            void let_index_correct();

            void update();
        };
        
        struct input_field : public input
        {
            ...
            input_field() : input() , index_color(BLACK) {}
            input_field(const input_field& other) = default;
            input_field(
                str_ex::ustring content,
                size_t index,
                text_ns::text draw_title,
